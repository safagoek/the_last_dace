...
def process_file_answers():
    """answer_file_path değeri dolu olan cevapları işler ve değerlendirir"""
    try:
        # Veritabanına bağlan
        ...
        # Dosya tipli cevapları bul (answer_file_path IS NOT NULL)
        cursor.execute("""
        ...
        print(f"{len(file_answers)} adet dosya cevabı değerlendirilecek.")
        ...
        for answer in file_answers:
            ...
            print(f"Soru ID: {question_id}, Dosya Yolu: {file_path}")
            
            # PDF'den metin çıkar
            answer_text = extract_text_from_pdf(file_path)
            
            if not answer_text:
                print(f"Cevap ID {answer_id} için dosya içeriği çıkarılamadı, atlanıyor")
                continue
        ...
        print(f"Dosya cevaplarını işlerken hata: {str(e)}")

def job():
    """Her saat başı çalışacak görev"""
    start_time = datetime.datetime.now()
    print(f"Dosya Cevapları Değerlendirme başladı: {start_time}")
    
    try:
        process_file_answers()
    except Exception as e:
        print(f"Genel hata: {str(e)}")
    finally:
        end_time = datetime.datetime.now()
        print(f"Dosya Cevapları Değerlendirme tamamlandı: {end_time}")
        print(f"Bir sonraki çalışma zamanı: {schedule.next_run()}")

if __name__ == "__main__":
    try:
        # Her saat başı çalışacak şekilde ayarla
        schedule.every().hour.at(":00").do(job)
        
        print("Program başlatıldı! Her saat başı çalışacak.")
        print(f"İlk çalışma zamanı: {schedule.next_run()}")
        
        # Programı hemen başlat (ilk çalışma)
        job()
